<template>
  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <div class="max-w-7xl mx-auto px-8 py-6 flex gap-6">
      ...
      <!-- Left sidebar — Post Type + Categories (Fix 2) -->
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700">
            <div class="flex items-center gap-3 cursor-pointer" @click="openPersonProfileById(activePost.user?.id)">
    ...
    <div v-if="personProfile" class="fixed inset-0 z-50 bg-black/70 flex items-center justify-center px-4" @click.self="personProfile = null">
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">

        <!-- Header banner -->
        <div class="bg-[#0d1117] rounded-t-2xl p-6 flex items-center gap-5 relative">
          <button @click="personProfile = null" class="absolute top-4 right-4 text-gray-400 hover:text-white text-lg">✕</button>
          <div class="w-20 h-20 rounded-full bg-indigo-600 flex items-center justify-center text-white text-3xl font-bold overflow-hidden shrink-0">
            <img v-if="personProfile.user.avatar" :src="`http://localhost:8000/storage/${personProfile.user.avatar}`" class="w-full h-full object-cover" />
            <span v-else>{{ personProfile.user.name?.charAt(0).toUpperCase() }}</span>
          </div>
          <div>
            <div class="flex items-center gap-2">
              <h2 class="text-white text-xl font-bold">{{ personProfile.user.name }}</h2>
              <span class="w-2 h-2 rounded-full" :class="personProfile.user.role === 'superadmin' ? 'bg-red-500' : personProfile.user.role === 'admin' ? 'bg-blue-500' : 'bg-green-500'"></span>
              <span class="text-xs" :class="personProfile.user.role === 'superadmin' ? 'text-red-400' : personProfile.user.role === 'admin' ? 'text-blue-400' : 'text-green-400'">{{ personProfile.user.role }}</span>
            </div>
            <p class="text-gray-400 text-sm mt-1">{{ personProfile.user.bio || 'No bio yet.' }}</p>
            <p class="text-gray-500 text-xs mt-1">Member since {{ formatDateLong(personProfile.user.created_at) }}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 border-b border-gray-100 dark:border-gray-700">
          <div class="text-center py-4 border-r border-gray-100 dark:border-gray-700">
            <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ personProfile.posts.length }}</p>
            <p class="text-xs text-gray-400">Posts</p>
          </div>
          <div class="text-center py-4">
            <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ personProfile.palettes.length }}</p>
        ...
        <div class="flex border-b border-gray-100 dark:border-gray-700">
          <button @click="personTab = 'posts'" class="flex-1 py-3 text-sm font-medium transition border-b-2" :class="personTab === 'posts' ? 'border-indigo-500 text-indigo-500' : 'border-transparent text-gray-400 hover:text-gray-600'">🖼 Posts</button>
          <button @click="personTab = 'palettes'" class="flex-1 py-3 text-sm font-medium transition border-b-2" :class="personTab === 'palettes' ? 'border-indigo-500 text-indigo-500' : 'border-transparent text-gray-400 hover:text-gray-600'">🎨 Palettes</button>
        </div>

        <!-- Posts tab -->
        <div v-if="personTab === 'posts'" class="p-4">
          <div v-if="personProfile.posts.length === 0" class="text-center py-8 text-gray-400 text-sm">No posts yet</div>
          <div class="grid grid-cols-3 gap-3">
            <div
              v-for="post in personProfile.posts" :key="post.id"
              @click="activePost = post; personProfile = null"
        ...
        <!-- Palettes tab -->
        <div v-if="personTab === 'palettes'" class="p-4">
          <div v-if="personProfile.palettes.length === 0" class="text-center py-8 text-gray-400 text-sm">No palettes yet</div>
          <div class="flex flex-col gap-3">
            <div v-for="palette in personProfile.palettes" :key="palette.id" class="rounded-xl overflow-hidden border border-gray-100 dark:border-gray-700">
              <div class="flex h-12">
    ...
    <div v-if="showCreateModal" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center px-4">
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg flex flex-col gap-4 p-6 max-h-[90vh] overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <!-- Category — hidden for palette type -->
        <select
    ...
    <div v-if="deleteTarget" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center px-4">
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 w-full max-w-sm [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
...
// Person profile modal
const personProfile = ref(null)
const personTab = ref('posts')
...
// Categories — Creation and Palette removed (Fix 2)
const categories = [
  { value: 'all',          icon: '🌐', label: 'All' },
...
async function openPersonProfile(person) {
  try {
    const { data } = await axios.get(`/api/users/${person.id}/profile`)
    personProfile.value = data
    personTab.value = 'posts'
  } catch (e) { console.error(e) }
}

async function openPersonProfileById(id) {
  if (!id) return
  try {
