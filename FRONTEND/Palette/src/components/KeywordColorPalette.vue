  ...
  <div class="min-h-screen bg-gray-50 p-8">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-7xl mx-auto">
      <div
        ...
        :class="selectedIds.has(palette.id)
          ...
          >
            ...
            <div
              ...
              :class="selectedIds.has(palette.id)
                ...
                : 'bg-gray-200 border-gray-300 group-hover:border-orange-300'"
            >
...
// Cycle through color counts: 3 → 5 → 7 → 9 → 3
const colorCounts = [3, 5, 7, 9]
const colorCountIndex = ref(1) // default 5
const colorCount = computed(() => colorCounts[colorCountIndex.value])
function cycleColorCount() {
  colorCountIndex.value = (colorCountIndex.value + 1) % colorCounts.length
}

// Cycle through palette counts: 4 → 8 → 12 → 16 → 4
const paletteCounts = [4, 8, 12, 16]
const paletteCountIndex = ref(1) // default 8
const paletteCount = computed(() => paletteCounts[paletteCountIndex.value])
function cyclePaletteCount() {
  paletteCountIndex.value = (paletteCountIndex.value + 1) % paletteCounts.length
}
